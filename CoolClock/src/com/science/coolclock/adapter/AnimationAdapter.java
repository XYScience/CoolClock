...
public abstract class AnimationAdapter extends BaseAdapterDecorator {
	...
	protected static final long DEFAULTANIMATIONDURATIONMILLIS = 600;
	...
	private void animateViewIfNecessary(int position, View view,
			ViewGroup parent) {
	...
	private void animateView(int position, ViewGroup parent, View view) {
		...
		if (mDecoratedBaseAdapter instanceof AnimationAdapter) {
			childAnimators = ((AnimationAdapter) mDecoratedBaseAdapter)
					.getAnimators(parent, view);
		} else {
	...
	private Animator[] concatAnimators(Animator[] childAnimators,
			Animator[] animators, Animator alphaAnimator) {
		Animator[] allAnimators = new Animator[childAnimators.length
				+ animators.length + 1];
		int i;
	...
	private long calculateAnimationDelay() {
		...
		if (numberOfItems + 1 < mLastAnimatedPosition) {
			...
			if (getAbsListView() instanceof GridView
				...
				delay += getAnimationDelayMillis()
						* ((mLastAnimatedPosition + 1) % ((GridView) getAbsListView())
								.getNumColumns());
			}
		} else {
			long delaySinceStart = (mLastAnimatedPosition + 1)
					* getAnimationDelayMillis();
			delay = mAnimationStartMillis + INITIALDELAYMILLIS
					+ delaySinceStart - System.currentTimeMillis();
		}
